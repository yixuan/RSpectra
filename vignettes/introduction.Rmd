...
[Spectra](http://yixuan.cos.name/spectra/) library, which is used to solve
...
If the matrix is known to be symmetric, calling
`eigs_sym()` is preferred since it guarantees that the eigenvalues are real.

To obtain eigenvalues of a square matrix `A`, simply call the corresponding
...
We first generate some random matrix and show some of the eigenvalues and
eigenvectors:
...
## Make it symmetric
A = crossprod(M)
## Show its largest 10 eigenvalues and associated eigenvectors
...
```{r}
## Use RSpectra to find the largest 5 eigenvalues
...
eigenvalues instead of small ones, so `which = "SM"` may involve much
computation or even not converging.
...
[This page](http://yixuan.cos.name/spectra/performance.html) gives some
